...
from morra.morra import SaMurra
...
def interact_play(app_id):
    ...
    while True:
        ...
        if global_state["state"] == 2:
            ...
            appclient_morra.call(SaMurra.commit, player.pk, commit=sha256(json.dumps({"guess": guess, "hand": hand, "nonce": nonce}).encode()).digest())
            print("Done!")
        elif global_state["state"] == 3 and ("player_state" not in local_state or local_state["player_state"] == 3):
            ...
            appclient_morra.call(SaMurra.reveal, player.pk, other=other, reveal=json.dumps({"guess": guess, "hand": hand, "nonce": nonce}))
            print("Done!")
            revealed=True
        elif global_state["state"] == 4 and local_state["player_state"] == 4:
            ...
            appclient_platform.call(GamePlatform.win_game, player.pk, app=app_id)
            print("Getting money...", end=" ", flush=True)
            appclient_morra.delete(player.pk, asset=berluscoin_id, creator=creator)
            print("Done!")
            return
        elif global_state["state"] == 5 and global_state["winner"] != codecs.encode(algosdk.encoding.decode_address(player.pk), 'hex').decode():
            print("You lost :(")
