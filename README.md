...
`opt_in(username)` This function can be called by any user to opt-in to the platform, with a username which will be saved in the local state of the user
...
`join_game(challenger, txn, app)` This function allows a player to participate in an existing game as a challenger.

`win_game(challenger, app)` This function can only be called by the winner of a game. If so, it increases the player's points according to the formula: **staked SKULLs / 100** saving them in the local state of the account
...
The owner of the match creates the smart contract and decides the amount of SKULLs to bet. From now on, any other user can join the game until a challenger is found. The challenger will have to deposit the amount of SKULLs decided by the creator. Now, each player has to choose a number from 0 to 5 which will represent the amount of fingers he decided to pick and a second number from the interval between 0 and 10 as his guess. At this point, each player has to send to the contract the digest obtained from the SHA256 hash function. The next phase is to reveal the players' plays: the players send their decisions as plain text. The contract recalculates the digest of their decision to verify their validity. Once the plays have been received, the contract will recognize the winning player and award him a point. The player who reaches 2 points first wins.

The two smart contracts were developed as finite state automata. The contract interacts with 2 actors: The Owner and the Challenger
...
`init()` Must be called by the contract creator to initialize the application, it must receive a transaction in which the ALGOs needed to instantiate the contract's account, and subsequent fees are paid
...
Following the rules of the game, the first two cards are given to the user, and the third card is given to the bank. At this point the player can pick cards until he reaches 21, busts or decides to stop. After that, the cards are given to the bank until they reach 17 or bust. Note that the user has no way to predict what card will be chosen (as they do not know the private key of the bank), and the bank has no way to influence what card will be chosen, as they must provide a (deterministic) signature of the data provided by the user. 

Functions:

`create(asset, bank, fee_holder)` This function can only be called by the game creator to create the contract. Requires to specify the game asset, the fee_holder to which the fees of any winnings will go and the address of the bank which will be saved in the contract state
...
`distribute_act(sig)` Is called by the bank to decide on a card used in the distribution phase. Can be called only after a distribution_req. To be called, the bank must sign the request made in the `distribute_req` function. This signature will be used to decide on what card will be picked. 
...
`hit_act(sig)` Is called by the bank to give a card to the player. Can be called only after a hit_req. To be called, the bank must sign the request made in the `hit_req` function. This signature will be used to decide on what card will be picked.
...
`stand_act(sig)` Is called by the bank to give a card to himself. Can be called only after a stand_req. To be called, the bank must sign the request made in the `stand_req` function. This signature will be used to decide on what card will be picked. 
...
Algo-Casino is a platform that brings classic casino games of poker, blackjack and others to the Algorand ecosystem. To play it is necessary to have the CHIP asset. In addition to the classic games it also offers a DeFi service.
...
For what concerns the contract themselves, most of the challenge in this project was making the game platform and the games communicate smoothly. To make our implementation cleaner, an obvious next step would consist in creating a base contract for all the games that implements the interface needed to enable that communication, as currently a lot of code is replicated between contracts.
